...
public class GateProxy {
    ...
    private static String FUNCTION = "CardInput";
...
//            KEY_PATH = "/home/liemnh/Desktop/Gate/JAVA/GateProxy/src/file/FPT/key";
        ...
        try {
            URLClassLoader CLASSLOADER = (URLClassLoader) GateProxy.class.getClassLoader();
...
//         response:   ErrorCode|Description|TransactionID |CardAmount|Time
            String response = checkTran(tran.getTransactionId().toString());
            if (response == null || response.isEmpty()) {
                ...
                if (msg[0].equals("00")) {
                    ...
                    tran.setResponseStatus(processGateCardService(new Integer(msg[0])));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//            String target = "homedirect" + tran.getTransactionId();
            ...
            String response = userCard(tran.getIssuer(), tran.getCardSerial(), tran.getCardCode(), tran.getTransactionId().toString(), target);
            if (response == null || response.isEmpty()) {
                tran.setResponseStatus(WebParameter.LOI_GOI_HAM_PROVIDER);
            } else {
                tran.setUseCardResponse(response);
    ...
    public static String userCard(String issuer, String cardSerial, String cardPin, String partnerTransactionID, String target) {
        String response = "";
        try {
...
//            System.out.println("-----xmlRequestData: \n" + xmlRequestData);
            response = service.ProcessRequest(SERVICE_ID, xmlRequestData);
...
//            System.out.println(prefixBuilder.toString());
    ...
    public static String checkTran(String partnerTransactionID) {
        String response = "";
        try {
            //Thuc hien tao chu ky theo tai lieu
            RequestData rq = new RequestData();
    ...
    private String processGateCardService(Integer status) {
        if (status == null) {
...
//        String result = userCard("GATE", cardSerial, cardPIN, partnerTransactionID, username);
        System.out.println("url:" + GateProxy.class.getResource(File.separator + "file" + File.separator + "FPT" + File.separator + "key"));
